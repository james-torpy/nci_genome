...
if init_bam_no == 0:
        ...
        with open(project_dir + 'bam_list.txt', 'r') as file_content:
            ...
            while line:
                ...
                if str(line) in open(project_dir + 'done_list.txt').read():
                    ...
                    if SAMPLES < 15:
                        print('Number of samples added so far: ' + str(bam_no) + 
                            '\n')
    ...
    #                            SAMPLES.append(str(sname))
        ...
        # convert SAMPLES from list to string, adding .bam:
        BAMS = [s + '.bam.gz' for s in SAMPLES]
        BAMS = [tape_dir + b for b in BAMS]
        BAMS_string = str(' '.join(BAMS))
        print('bams are ' + str(BAMS))

        # get bam file from tape:
        os.system('cd ' + bwa_dir + '; mdss stage -w ' + BAMS_string)
        for bamfile in BAMS:
            print('Fetching ' + bamfile + ' from tape\n')
            os.system('cd ' + bwa_dir + '; mdss get ' + bamfile)

        for s in SAMPLES:
            print('gunzipping ' + bwa_dir + str(s) + '.bam.gz')
            os.system("gunzip " + bwa_dir + str(s) + '.bam.gz')
